...
if [[ -f "${RAW_IMAGE}" ]]; then
  ...
  rm -f "${RAW_IMAGE}.xz" "${RAW_IMAGE}"

  /usr/bin/wget -O "${RAW_IMAGE}.xz" "https://factory.talos.dev/image/${SCHEMATIC_ID}/${TALOS_VERSION}/nocloud-amd64.raw.xz"
...
/usr/sbin/qm set "${VMID}" --efidisk0 sdc-storage:1,efitype=4m,pre-enrolled-keys=0

/usr/sbin/qm importdisk "${VMID}" "${RAW_IMAGE}" sdc-storage

/usr/sbin/qm set "${VMID}" --scsihw virtio-scsi-pci --scsi0 "sdc-storage:${VMID}/vm-${VMID}-disk-1.raw"

/usr/sbin/qm resize "${VMID}" scsi0 50G

/usr/sbin/qm set "${VMID}" --boot order=scsi0
/usr/sbin/qm set "${VMID}" --serial0 socket
/usr/sbin/qm set "${VMID}" --vga std
/usr/sbin/qm set "${VMID}" --agent enabled=1
